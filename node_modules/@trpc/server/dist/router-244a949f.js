...
var codes = require('./codes-731f0a7c.js');
...
function migrateProcedure(oldProc, type) {
    ...
    const inputParser = resolveHTTPResponse.getParseFnOrPassThrough(def.inputParser);
    const outputParser = resolveHTTPResponse.getParseFnOrPassThrough(def.outputParser);
    const inputMiddleware = resolveHTTPResponse.createInputMiddleware(inputParser);
    const builder = resolveHTTPResponse.createBuilder({
        inputs: [
            def.inputParser
        ],
        middlewares: [
            ...def.middlewares,
            inputMiddleware,
            resolveHTTPResponse.createOutputMiddleware(outputParser), 
        ],
        meta: def.meta,
